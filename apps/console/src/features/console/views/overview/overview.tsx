  ...
  return (
    <span
      className={cn(
        ...
        change > 0 === goodWhenUp ? "text-emerald-500" : "text-destructive",
      )}
    >
  ...
  return (
    <Card className="gap-0 py-0">
      <CardContent className="flex min-h-[130px] items-center gap-3 p-4">
        <div
          className={cn("grid size-11 shrink-0 place-items-center rounded-xl [&_svg]:size-5", tone)}
        >
          {icon}
        </div>
        <div className="min-w-0">
          <p className="truncate text-xs font-medium text-muted-foreground" title={label}>
            {label}
          </p>
          <p
          ...
          <p className="mt-0.5 text-xs">
            <Change current={current} previous={previous} goodWhenUp={goodWhenUp} />{" "}
  ...
  return (
    <Card className="h-full min-h-[230px] min-w-0 gap-0 py-0">
      <CardContent className="p-4">
        ...
        <ChartContainer
          config={{ count: { label: "Sessions", color: "var(--primary)" } }}
...
function OutcomeCard({ reasons }: { reasons: OperatorOverview["tickets"]["resolvedByReason"] }) {
  const rows = outcomeReasons.map(({ key, label, color, hint }) => ({
    label,
    color,
    hint,
    count: reasons[key],
      ...
      <CardContent className="p-4">
        ...
        {total === 0 ? (
          <p className="grid h-40 place-items-center text-xs text-muted-foreground">
            No resolved Tickets in this range.
          </p>
        ) : (
          <div className="mt-4 flex items-center gap-4">
            <div
              ...
              role="img"
              aria-label={`${formatCount(total)} resolved Tickets by resolution reason`}
            >
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span
                            className="truncate text-muted-foreground underline decoration-dotted"
                            title={row.label}
                          >
                            {row.label}
                          </span>
        ...
        {total === 0 ? (
          ...
          <div className="mt-6 space-y-5">
            {rows.map((row) => (
              <div
                key={row.key}
                className="grid grid-cols-[5rem_minmax(0,1fr)_2.5rem_2.5rem] items-center gap-2 text-xs"
              >
                <span className="truncate text-muted-foreground">{row.label}</span>
                <div
                  className="h-3 overflow-hidden rounded bg-muted"
                  role="img"
                  aria-label={`${row.label}: ${formatCount(row.count)} Sessions`}
                >
                  <div
                    className="h-full rounded"
                    style={{
                      width: `${(row.count / total) * 100}%`,
                      background: channelColors[row.key],
                    }}
                  />
                </div>
          ...
          <>
            ...
            {overview.dataUpdatedAt ? (
              <span className="text-xs text-muted-foreground">
                Updated {dateTimeFormat.format(new Date(overview.dataUpdatedAt))}
              </span>
            ) : null}
          </>
        }
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            <Metric
              ...
              hint="Had activity in selected period"
              value={formatCount(data.workspaces.active)}
              current={data.workspaces.active}
              previous={data.previous.activeWorkspaces}
            ...
            <Metric
              ...
              current={data.workspaces.new}
              previous={data.previous.newWorkspaces}
              tone="bg-primary/10 text-primary"
            />
            <Metric
            ...
            <Metric
              ...
              hint="Paid top-ups in selected period"
              value={formatIdr(data.revenueIdr)}
              current={data.revenueIdr}
          ...
          <section
            ...
            className="grid items-stretch gap-4 lg:grid-cols-2 2xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1.1fr)_minmax(0,0.95fr)] [&>*]:min-w-0"
          >
          ...
          <section
            aria-label="Operator priorities"
            className="grid items-stretch gap-4 2xl:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)_minmax(0,1fr)] [&>*]:min-w-0"
          >
              ...
              <CardContent className="p-0">
                <div className="flex items-center justify-between gap-2 border-b px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-sm font-semibold">Most active workspaces</h2>
                      <p className="text-xs text-muted-foreground">Sessions in selected period</p>
                    </div>
                  </div>
                  <Link to="/workspaces" className="shrink-0 text-xs text-primary hover:underline">
                    View all →
                  </Link>
                </div>
                {data.topWorkspaces.length === 0 ? (
                  <p className="grid min-h-40 place-items-center text-xs text-muted-foreground">
                    No Sessions in this range.
                  </p>
                ) : (
                  <div className="divide-y px-4">
                    <div className="grid grid-cols-[minmax(0,1fr)_3.5rem_5rem] gap-2 py-2 text-xs text-muted-foreground">
                      <span>Workspace</span>
                      <span className="text-right">Sessions</span>
                      <span className="text-right">Δ previous</span>
                    </div>
                    {data.topWorkspaces.map((workspace) => (
                      <div
                        key={workspace.id}
                        className="grid grid-cols-[minmax(0,1fr)_3.5rem_5rem] items-center gap-2 py-2.5 text-xs"
                      >
                        <Link
                          to="/workspaces/$workspaceId"
                          params={{ workspaceId: workspace.id }}
                        ...
                        <span className="text-right tabular-nums">
                          {formatCount(workspace.sessions)}
                        </span>
                        <span className="text-right">
                          <Change
                            current={workspace.sessions}
                            previous={workspace.previousSessions}
                          />
                        </span>
                      </div>
                    ))}
              ...
              <CardContent className="p-0">
                <div className="flex items-center justify-between gap-2 border-b px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-sm font-semibold">Recent operator actions</h2>
                      <p className="text-xs text-muted-foreground">
                      ...
                      <div key={action.id} className="flex items-center gap-2 py-2.5 text-xs">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-muted-foreground">
                            {action.workspace?.name ?? "Workspace"}
                            {action.type === "TOP_UP" && typeof action.payload.credits === "number"
                              ? ` · +${formatCount(action.payload.credits)} Credits`
                              : ""}
                          </p>
                        </div>
                        <time
                        ...
                        >
                          {dateTimeFormat.format(new Date(action.createdAt))}
                        </time>
                      </div>
                    ))}
                  </div>
