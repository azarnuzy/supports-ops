...
async function publishStatus(ticketId: string, attachmentId: string, status: "PROCESSING" | "READY" | "FAILED", failureReason?: string) {
  publisher ??= new Redis(process.env.REDIS_URL ?? "redis://localhost:16379", { maxRetriesPerRequest: null });
  await publisher.publish(
    `supportops:ticket:${ticketId}`,
    JSON.stringify({ type: "attachment.updated", data: { attachmentId, failureReason, processingStatus: status } }),
  );
}

export async function processAttachmentJob(job: { data: AttachmentProcessJob }) {
  const attachment = await prisma.attachment.findFirst({
    where: { deletedAt: null, id: job.data.attachmentId, ticketId: job.data.ticketId, workspaceId: job.data.workspaceId },
  });
  if (!attachment) return;

  await prisma.attachment.update({ data: { failureReason: null, processingStatus: "PROCESSING" }, where: { id: attachment.id } });
  ...
  try {
    ...
    await prisma.attachment.update({ data: { extractedText, processingStatus: "READY" }, where: { id: attachment.id } });
    ...
    await prisma.attachment.update({ data: { failureReason, processingStatus: "FAILED" }, where: { id: attachment.id } });
...
async function extractAttachment(storageKey: string, mimeType: string) {
  ...
  if (!ingestionConfig.mistralApiKey) throw new Error("Configure MISTRAL_API_KEY to process attachments.");
  ...
  const response = await fetch("https://api.mistral.ai/v1/ocr", {
    ...
    headers: { Authorization: `Bearer ${ingestionConfig.mistralApiKey}`, "Content-Type": "application/json" },
    method: "POST",
  });
...
async function requestReply(ticketId: string, workspaceId: string) {
  if (!apiConfig.workerToken) throw new Error("Configure INTERNAL_WORKER_TOKEN to generate attachment replies.");
  const response = await fetch(`${apiConfig.internalUrl.replace(/\/$/, "")}/internal/tickets/${ticketId}/generate`, {
    body: JSON.stringify({ workspaceId }),
    headers: { "Content-Type": "application/json", "x-supportops-worker-token": apiConfig.workerToken },
    method: "POST",
  });
