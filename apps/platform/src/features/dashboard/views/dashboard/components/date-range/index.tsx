...
import { formatRangeLabel, RANGE_PRESETS, toISODate, trailingRange } from "../../dashboard.utils";
import type { DateRangePickerProps } from "./index.types";

/** Structural mirror of react-day-picker's DateRange; the platform does not
 * depend on that package directly. */
type CalendarSelection = { from?: Date; to?: Date };

export default function DateRangePicker({ range, onChange, className }: DateRangePickerProps) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          className={cn("h-8 cursor-pointer gap-1.5 rounded-full px-3 text-xs", className)}
        >
