...
function Attachment({
  ...
  state = "done",
  ...props
}: React.ComponentProps<"div"> & { state?: "uploading" | "processing" | "error" | "done" }) {
  return (
    <div
      data-slot="attachment"
      data-state={state}
      className={cn(
        "flex max-w-full items-center gap-2 rounded-xl border bg-card p-2 text-sm data-[state=error]:border-destructive/40",
        className,
      )}
      {...props}
    />
