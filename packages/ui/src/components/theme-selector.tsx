...
function ThemePresetSelector({ className }: { className?: string }) {
  const [preset, setPreset] = React.useState<ThemePreset>(() =>
    typeof window === "undefined"
      ? "default"
      : (window.localStorage.getItem("theme-preset") as ThemePreset) || "default",
  );
  React.useEffect(() => {
  ...
  return (
    <select
      ...
      className={cn(
        "h-8 rounded-md border border-border bg-background px-2 text-xs font-medium",
        className,
      )}
    >
      {presetOptions.map((option) => (
        <option key={option} value={option}>
          {option === "default" ? "Default" : option.replace("-", " ")}
        </option>
      ))}
    </select>
  );
