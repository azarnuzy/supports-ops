...
Agent telemetry goes through `@anvia/otel` to an OTLP endpoint, alongside the application traces that `packages/logger` already emits, rather than to Anvia's own Lens backend. The OTLP destination is configuration, not architecture: Langfuse Cloud to begin with, swappable for Phoenix, SigNoz, Grafana, or self-hosted Lens by changing an environment variable.
