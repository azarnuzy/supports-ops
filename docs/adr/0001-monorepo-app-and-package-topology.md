...
**Update (2026-09-24):** `packages/shared` was removed. It never held anything — cross-app types reach the frontend through Hono RPC in `@repo/api-client`, and shared helpers live in the package that owns their concept. A new package is added only when an ADR-level boundary calls for it, not as a catch-all.
