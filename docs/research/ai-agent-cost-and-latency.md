...
## 1c. Baseline B2 (measured 2026-09-16, after #181 and #184)

Every suite below was run against the same eval Workspace on 2026-09-16 (`pnpm eval:ai-agent <suite>`), one process per suite, sequentially, at commit `39da01c` — after 6.10/6.11 (#179/#180) and after the reply-prompt conflict/escalation rules and the static Tool loadout scoping described in 7.2 and 7.4 below. B0 and B1 above are unchanged. The development constraint noted for #179's and 7.4's prior status (no `EVAL_WORKSPACE_ID`/model credentials in the implementation worktree) no longer applies — this session has both.

| Suite | Cases | Pass | Target tokens | Tokens/case | Wall time | ms/case | Cached | TTFT p50/p95 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `contains` | 6 | 4 | 313,419 | 52,237 | 91,960 ms | 15,327 | 78% | 7,943 / 41,177 ms |
| `faithfulness` | 3 | 1 | 71,619 | 23,873 | 85,093 ms | 28,364 | 77% | 7,569 / 8,241 ms |
| `decision` | 14 | 14 | 259,658 | 18,547 | 115,007 ms | 8,215 | 81% | 6,271 / 21,207 ms |
| `tool` | 7 | 7 | 456,688 | 65,241 | 106,719 ms | 15,246 | 71% | 13,935 / 33,244 ms |
| `visibility` | 6 | 6 | 132,901 | 22,150 | 74,365 ms | 12,394 | 83% | 9,439 / 32,616 ms |
| `negativeControl` | 1 | 0 | 23,713 | 23,713 | 5,976 ms | 5,976 | 100% | 5,975 / 5,975 ms |
| `language` | 3 | 3 | 72,210 | 24,070 | 28,195 ms | 9,398 | 92% | 9,433 / 10,461 ms |
| `relevancy` | 3 | 3 | 72,404 | 24,135 | 81,193 ms | 27,064 | 61% | 6,579 / 8,845 ms |
| `gEval` | 23 | 18 | 764,052 | 33,220 | 415,688 ms | 18,073 | 74% | 7,787 / 25,258 ms |

`visibility`'s Case count grew from 4 to 6 between B1 and this run (Cases added by other work in the same window, not by #181 or #184); all 6 pass. Two suites outside B0/B1's original nine also exist now (`exactmatch`, 2/2 pass; `retrieval`, unchanged from the #182 baseline in §7.7 — still recall-strong/precision-weak by design, out of scope here).

**Quality floor: held, and two named B1 failures now pass.** `visibility`, `language`, `relevancy` stay at 100%. `negativeControl` still fails by design. `contains` is unchanged at 4/6 (`tool-jordan-price`, `tool-rolex-price` — the strict-schema regression named in 1a, still out of scope). `faithfulness` is unchanged at 1/3, and the same two Cases fail: `edge-no-first-scan` (score 0, the #179/6.10 grounding fix has **not** resolved it — the reply still frames the Customer's own "three business days" as if it were the retrieved threshold; noted here since this run happens to measure it, but re-triaging #179 is not in this work's scope) and `edge-original-shipping-refund` (score 0.67, unchanged, same judge-variance case named in 1a). `decision` improved from 12/14 (B1) to **14/14**: `common-return-window` and `escalation-two-completed-charges` — the two Cases #181 targets — both pass. `tool` stays at 7/7 (already fixed and measured by #180/6.11). `gEval` stays at 18/23 (already measured by 6.11); the specific failing Case names shifted slightly (`edge-shoes-worn-outside` appears in place of one previously-named Case) which is the same eval non-determinism the doc has flagged before, not a new regression — pass count is identical.

**#181 acceptance criteria: met.** `common-return-window` and `escalation-two-completed-charges` pass; `gEval` improved against B1 (16/23 → 18/23, unchanged from 6.11's prior measurement, now reconfirmed on a live run that also includes 7.2's remaining prompt changes); no suite regressed; the five real gEval failures and the two faithfulness failures are all previously-named causes (strict-schema regression, pre-existing quality variance, or #179's still-open gap), not new ones.

**#184 acceptance criteria: met, with the manifest saving measured at the correct level rather than in the blended suite average.** `selectToolLoadout` (`packages/ai-agent/src/tools.ts`) was run directly against this Workspace's 14 assigned Tools (`cancel_cart`, `cancel_checkout`, `complete_checkout`, `create_cart`, `create_checkout`, `get_cart`, `get_checkout`, `getInvoiceStatus`, `get_order`, `get_product`, `lookup_catalog`, `search_catalog`, `update_cart`, `update_checkout`) and against every real Customer message in the 78-Case eval set:

- The `support` loadout drops the 7 cart/checkout mutation Tools, leaving 7 read-only/unrelated Tools. Measuring description + JSON Schema text the same way section 2 did (~4 chars/token): full manifest ≈ **13,051 tokens**, `support` loadout ≈ **3,078 tokens** — a **~76% reduction, ~9,973 tokens**, for any turn that doesn't carry purchase intent.
- 77 of 78 real eval-Case Customer messages route to `support`. The one exception (`abstention-loyalty-programme`, "How many points does your loyalty programme give *per purchase*?") is a literal match on the word "purchase" in `PURCHASE_INTENT` — a false positive, low-impact (not in a suite whose pass/fail depends on Tool scoping), left as-is.
- This reduction is **not** visible as a clean before/after in the suite-level `tokens/case` numbers above, because #181's reply-prompt additions (the `CONFLICTING_KNOWLEDGE` rule, `sourceTitle` threading, and the later escalation-avoidance fixes) grew the static system-prompt tail in the same commit window, and because `decision`'s two newly-passing Cases now make the Tool calls their correct answer requires, where before they gave up early. Both effects land in the same `tokens/case` column as the loadout change and cannot be separated from it in a blended average — this is why the manifest saving is reported at the code level above instead of claimed from the suite table.
- Cached-token ratio did not fall: no suite's cached percentage collapsed toward 0% (the signature of a broken prefix); on an isolated no-Tool-call `decision` Case, cached tokens are 10,926 / 10,944 = **99.8%**, matching B1's 99.7–99.9% figure for the same class of Case.
- `tool` (7/7) and `decision` (14/14) did not regress — no Case failed because a withheld Tool was needed.
...
### 6.12 A second pass at `edge-no-first-scan`, root cause narrowed but not closed — #179

B2 (§1c) showed 6.10's prompt change had not actually fixed `edge-no-first-scan`. A second, targeted round against `pnpm eval:ai-agent --id=edge-no-first-scan` (single-Case runs, not the full suite, for fast iteration) found the original diagnosis was incomplete:

- **First real bug found: the model was still stating a Customer-supplied duration, and adding a plausible-sounding definition of "label created" ("shipment prepared, carrier may not have scanned it yet") even on turns where that definition was not in this turn's actual `searchKnowledge` result.** 6.10's grounding paragraph forbade this in general language but did not stop it in practice. `packages/ai-agent/src/prompts/reply.ts` was tightened: the rule against defining a term now states explicitly that a definition the model recalls as generally true is still model knowledge, not this turn's Tool Result, and forbidding it applies "even when it happens to be accurate" — closing the loophole where a plausible, true-sounding explanation was treated as acceptable because it wasn't obviously wrong.
- **A second, separate cause surfaced during iteration and was tried and reverted**: this Knowledge Source publishes two different thresholds for superficially similar situations — a "beyond two business days" rule specific to the no-first-scan status (§6.1), and an unrelated "3+ business days, escalate immediately" row in a general decision-matrix table. A rule telling the model to prefer whichever passage most specifically named the Customer's exact status made results worse (3 of 3 single-Case runs scored 0, down from a mixed 0.2–0.5), so it was dropped rather than kept on the theory that it should have helped.
- **After the fix that stuck**: `pnpm eval:ai-agent --id=edge-no-first-scan` passed cleanly (score 1) on one run, and failed at 0.5 on two more — better than 6.10's baseline (which failed at 0–0.33) but still below the 0.7 threshold, and the Case remains genuinely marginal rather than fixed.
- **What the remaining 0.5-score failures show: a structural mismatch, not a wording gap.** `replyPrompt` requires every ESCALATE to state that the Ticket is being passed to a Human Agent (line 34, a platform-level format rule, not a Knowledge claim). The `faithfulness` judge scores every sentence in `output` against `retrieved`, including that mandatory procedural sentence — which by definition has no supporting Knowledge passage, since it isn't a Knowledge fact. On the runs that reach ESCALATE with an otherwise-correct, grounded explanation, this one unavoidable sentence is enough to keep the score under 0.7. This is the same class of harness/metric shape problem 6.2 fixed for CLARIFY turns with no retrieval — but fixing it here means teaching the `faithfulness` judge (or its `retrievalContext`, `apps/api/src/evals/run.ts:86`) to exclude the mandated ESCALATE/RESOLVE framing sentence from what it grades, not another prompt change. That is a harness change shared by every `faithfulness` Case, not a `reply.ts`-only fix, and was not attempted here.

Verify: `pnpm --filter @repo/ai-agent test` (46 passing) and `tsc --noEmit` pass. `pnpm eval:ai-agent faithfulness` re-run shows no regression against B2: `common-split-shipment` still passes (0.75), `edge-original-shipping-refund` still fails at the same 0.67 score. `edge-no-first-scan` itself is left failing (0–0.5 across repeated single-Case runs) — the prompt change is kept because it measurably improved the Case and regressed nothing, but #179 is **not** closed: the acceptance criterion ("the Case passes") is not met, and closing it would require the harness-side judge change named above, which is out of scope for this pass.
...
- `edge-no-first-scan` — **still open, see 6.12.** 6.10's prompt change didn't fix it (B2, §1c); a second, targeted pass (6.12) fixed the actual bug it caused (a plausible-but-ungrounded definition of "label created") and the Case now scores 0.5–1 instead of 0–0.33, but the remaining gap is a harness/metric shape issue — the `faithfulness` judge penalizes the ESCALATE framing sentence `replyPrompt` requires — not something another prompt edit can close. #179 stays open pending that judge-side change.
...
- Answer-quality Cases (`common-return-window`, `escalation-two-completed-charges`) — **done and measured, see §1c (#181).** Both pass in B2's `decision` suite (14/14, up from B1's 12/14); `gEval` holds at 18/23 (no regression); no other suite regressed against B1.
...
**Implemented and measured, see §1c.** The model receives one of two fixed loadouts: `support` contains every assigned Tool except cart/checkout mutation Tools, while `purchase` contains every assigned Tool. A purchase request selects `purchase`; once the Session history contains a purchase flow, later turns keep it so confirmations cannot lose the Tool they authorize. This keeps only two stable manifest prefixes instead of computing an arbitrary Tool set per message. Measured directly against this Workspace's 14 assigned Tools: the `support` loadout is ~76% smaller than the full manifest (~13,051 → ~3,078 estimated tokens), and 77 of 78 real eval Customer messages route to it. `tool` and `decision` did not regress (7/7, 14/14) and no suite's cached-token ratio fell. The suite-level `tokens/case` numbers do not show this saving cleanly because #181's prompt growth shipped in the same window — see §1c for why, and for the isolated measurement that avoids that confound.
