...
- `edge-no-first-scan` — **prompt change shipped in 6.10 (#179), but B2 (§1c) shows it still fails**, same shape as before (the reply still restates the Customer's own "three business days" as if it were the retrieved threshold). Re-triaging #179 against this new evidence is out of scope for the current work and is left as an open item, not silently marked done.
