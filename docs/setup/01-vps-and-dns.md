...
| Hostname | Purpose |
| --- | --- |
| `app.support.azarnuzy.com` | Dashboard |
...
For the example output above, this is the exact mapping. The IP address and file path are examples only; use the values printed on your machine:

| GitHub field | Value to enter for the example |
| --- | --- |
| Secret `VPS_HOST` | `203.0.113.42` |
| Variable `VPS_USER` | `ubuntu` |
| Variable `VPS_PORT` | `22` |
| Existing local admin key | `~/.ssh/tencent-lighthouse` — do **not** upload this personal key |

For example, if your command instead prints `user root`, `hostname 43.128.12.34`, and `port 2222`, enter `43.128.12.34` for `VPS_HOST`, `root` for `VPS_USER`, and `2222` for `VPS_PORT`.
...
For a concrete example of the two multi-line secrets: after running the commands above, run the following commands locally. They copy the values; they do not print secret content to the terminal.

```bash
# Copy the entire private key, including BEGIN/END lines, then paste it into
# GitHub secret VPS_SSH_PRIVATE_KEY.
pbcopy < "$HOME/.ssh/supportops-github-actions"

# Copy every line generated by ssh-keyscan, then paste it into
# GitHub secret VPS_KNOWN_HOSTS.
pbcopy < /tmp/supportops-vps-known-hosts
```

The private-key secret will look structurally like this; paste all of its real lines, not these placeholders:

```text
-----BEGIN OPENSSH PRIVATE KEY-----
<many secret base64 lines>
-----END OPENSSH PRIVATE KEY-----
```

The known-hosts secret will look structurally like this; it is not secret, but must be the real output for the VPS:

```text
|1|<hashed-host>|<hashed-salt> ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAI<server-public-key>
```
...
curl --fail --location https://app.support.azarnuzy.com/
...
Confirm the four application HTTPS checks again. The landing page at `support.azarnuzy.com` is deployed from the separate `supportops-landingpage` repository. A successful rollback replaces `.release.env` with the restored SHA; no image is built on the VPS.
