...
The model gateway is [OpenRouter](https://openrouter.ai). One API key covers completions (classification, replies, Suggested Reply, Escalation Summary) and embeddings (Knowledge Source publishing, retrieval).
...
`EMBEDDING_MODEL` defaults to `openai/text-embedding-3-small` (1536 dimensions) — OpenRouter's provider-prefixed form, not the bare OpenAI model id. `LLM_MODEL_FAST` defaults to `openai/gpt-4.1-nano` and is the fast model tier — classification, title, category, priority, language. `LLM_MAIN_REASONING_EFFORT` and `LLM_MAIN_MAX_OUTPUT_TOKENS` tune the reply tier and may be left empty to use provider defaults (an empty string means unset, not zero).

There is no environment variable for the reply model. Each AI Agent's **Agent Model** (`AiAgent.agentModel`) picks it from the code-defined Model Catalog in `apps/api/src/modules/ai-agent/model-catalog.ts`, validated by the AI settings API and chosen by an Admin in AI settings — see [ADR-0022](../adr/0022-agent-model-from-code-defined-catalog.md). An Agent Model dropped from the catalog falls back to the catalog's default at call time.
