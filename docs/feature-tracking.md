...
| `/tickets/mine` | Human Agent | Siap dicoba | Melihat percakapan Ticket yang telah di-Claim, meminta Suggested Reply dari AI Copilot, mengeditnya, mengirim balasan, dan melakukan Resolution. Pesan Customer serta perubahan Ticket masuk otomatis tanpa refresh. | Admin belum dapat membalas atau melakukan Resolution atas nama Human Agent. Suggested Reply membutuhkan `OPENROUTER_API_KEY` dan Knowledge Source yang telah dipublish. |
