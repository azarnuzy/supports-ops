...
| `/chat` | Pengguna masuk | Sebagian siap | Vertical slice pertama dari Unified Ticket Workspace ([#43](https://github.com/azarnuzy/supports-ops/issues/43)): melihat Ticket pada scope Mine (milik sendiri) dengan data nyata, membuka Ticket di URL stabil `/chat/tickets/:ticketId` (bertahan saat refresh), membaca transcript Message dan Escalation Summary, meminta Suggested Reply dari AI Copilot (tetap eksplisit dan dapat diedit, tidak pernah terkirim otomatis), mengirim balasan teks, serta melakukan Resolution dengan konfirmasi — Ticket tetap terlihat sebagai transcript read-only setelahnya. Panel Details dan Activity menampilkan Customer Identity, status/owner/category/priority, Escalation Reason, data Resolution, dan riwayat AI Activity yang dapat dibaca manusia. | Baru mencakup scope Mine; belum ada scope Unassigned/All atau operasi Admin di halaman ini ([#45](https://github.com/azarnuzy/supports-ops/issues/45)–[#47](https://github.com/azarnuzy/supports-ops/issues/47)). Belum mendukung Attachment. Suggested Reply membutuhkan `OPENROUTER_API_KEY` dan Knowledge Source yang telah dipublish. Route `/tickets*` tetap tersedia sampai workspace baru mencapai feature parity ([#53](https://github.com/azarnuzy/supports-ops/issues/53)). |
